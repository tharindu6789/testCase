...
public class Rules2 {
	...
	public String validate(String ruletype, String paragraph) {
		...
		try {
			if (model != null) {
				...
				if (tagger != null) {
					...
					preList = new Rules2().Rule1(sentences, tagger);

					if (preList.size() > 1) {
						ArrayList<String> ruleList2 = new Rules2().Rule2(preList, tagger);
						preList.clear();
						preList.addAll(ruleList2);

						if (preList.size() > 1) {
							ArrayList<String> ruleList3 = new Rules2().Rule3(preList, tagger);
							preList.clear();
							preList.addAll(ruleList3);
							if (preList.size() > 1) {
								ArrayList<String> ruleList4 = new Rules2().Rule4(preList, tagger);
								preList.clear();
								preList.addAll(ruleList4);
								if (preList.size() > 1) {
									ArrayList<String> ruleList5 = new Rules2().Rule5(preList, tagger);
									preList.clear();
			...
			for (int i = 0; i < whitespaceTokenizerLine.length; i++) {
				...
				if (predefinedList.contains(word + "/" + tag)) {
					if (ruleList.size() == 0 || !ruleList.contains(sentence)) {
						sentence=sentence.replace(word, "BBB "+word);
